...
bool CS_ZQJ::TriggerJudge(CPlayer * actPlayer, CPlayer *defPlayer)
{
	int actsite=CGameEngine::GetGE()->GetMoves()->GetSite(actPlayer->m_CurMove,actPlayer->m_CurF);

	if(actsite==10||actsite==12)//站立轻脚
	{
		actPlayer->m_CurMove=8;
		actPlayer->m_CurCF=0;
		actPlayer->m_CurF=0;
...
void CS_ZQJ::TriggerControl(CPlayer * actPlayer, CPlayer *defPlayer)
{

}

bool CS_ZQJ::HitJudge(CPlayer * actPlayer, CPlayer *defPlayer)
{

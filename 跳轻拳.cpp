...
bool CS_TQQ::TriggerJudge(CPlayer * actPlayer, CPlayer *defPlayer)
{
...
void CS_TQQ::TriggerControl(CPlayer * actPlayer, CPlayer *defPlayer)
{
...
bool CS_TQQ::HitJudge(CPlayer * actPlayer, CPlayer *defPlayer)
{
	int defsite=CGameEngine::GetGE()->GetMoves()->GetSite(defPlayer->m_CurMove,actPlayer->m_CurF);
	if(defsite==12||defsite==14||defPlayer->m_HurtS==this)
	{
		actPlayer->m_CurSCF=0;
		return false;
	}
	...
	defPlayer->m_CurSCF=0;
	defPlayer->m_HurtS=this;
	defPlayer->m_CurHCF=0;
	CGameEngine::GetGE()->GetAM()->PlayOnceAudio("绊脚");	
	actPlayer->m_TX.push_back(new CTX_TPZ(111,actPlayer->m_x+(actPlayer->m_Face==_FACE_RIGHT?45:-45),actPlayer->m_y-60,actPlayer->m_Face==_FACE_RIGHT?_MIRROR_NONE:_MIRROR_LR));
	return true;
}

//技能命中后对对方的持续控制 用于准确控制对方的动作
void CS_TQQ::HitControl(CPlayer * actPlayer, CPlayer *defPlayer)
{
	if(defPlayer->m_CurHCF>5)
